        ...
        <section className="py-16">
            <div className="mx-auto max-w-6xl px-6 text-center">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-7">
                    {sponsors.map((s, i) => (
                            <a key={s.id} href={s.website} target="_blank" rel="noreferrer" className={`drop-card p-8 sponsor-card flex flex-col items-center justify-center text-center min-h-56 `} style={{ animationDelay: `${i * 80}ms` }} aria-label={s.id} >
                                <div className={s.logoWrapClass ?? ""}>
                                    <img src={s.logo} alt={s.id} className={`w-auto object-contain ${s.logoClass ?? "max-h-16"}`} />
                                </div>
