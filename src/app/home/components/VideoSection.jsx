export default function VideoSection({ title, description, videoSource }) {
    return (
        <section className="relative w-full h-screen overflow-hidden isolate bg-ocean-dark">
            {videoSource && (
                <video src={videoSource} className="absolute inset-0 w-full h-full object-cover [filter:brightness(0.55)] [transform:translateZ(0)] will-change-transform" autoPlay muted loop playsInline preload="auto" />
            )}
